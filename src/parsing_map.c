...
void parse_map_lines(t_game *game, char *line, int fd, int *i)
{
    ...
    printf("Allocated map[%d]: %p - Content: %s\n", *i, game->map[*i], game->map[*i]);
    if (!game->map[*i])
...
void parse_map(t_game *game, char *line, int fd)
{
    ...
    printf("Allocated game->map: %p\n", game->map);
    parse_map_lines(game, line, fd, &i);
    game->map_height = i;
    game->map[i] = NULL;
    printf("Map height set to: %d\n", game->map_height);
    set_map_width(game);
    printf("Calling validate_map...\n");
    validate_map(game);
    printf("validate_map completed\n");
}
...
static void handle_line(char *line, t_game *game, int *map_started, int fd)
{
    ...
    {
        *map_started = 1;
        printf("Starting map parsing with line: %s\n", line);
...
void parse_cub_file(char *filename, t_game *game)
{
    ...
    printf("parse_cub_file completed\n");
}
