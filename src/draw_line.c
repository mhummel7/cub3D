...
void	initialize_values(t_player *player, t_draw_line_variables *vars,
...
void	draw_line(t_player *player, t_game *game, float dest_x, float dest_y)
{
	...
	while (1)
	{
		if (vars.init_x >= 0 && vars.init_x < game->window_width
			&& vars.init_y >= 0 && vars.init_y < game->window_height)
			mlx_put_pixel(game->dynamic_layer, vars.init_x, vars.init_y,
				0xFF0000FF);
